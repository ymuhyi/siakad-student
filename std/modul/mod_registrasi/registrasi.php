...
?>
	<div class='message success'>
		<h5>Success!</h5>
		<p>Registrasi ulang berhasil disimpan.</p>
	</div>
<?php
}
if ($_GET['code'] == 2){
?>
	<div class='message success'>
		<h5>Success!</h5>
		<p>Registrasi ulang berhasil diubah.</p>
	</div>
<?php
}
if ($_GET['code'] == 3){
?>
	<div class='message success'>
		<h5>Success!</h5>
		<p>Registrasi ulang berhasil dihapus.</p>
	</div>
<?php
}
?>
...
switch($_GET['act']){
	...
	$sem = $data_mhs['semester_now'] + 1;
	
	$date_now = tgl_indo(date('Y-m-d'));
	
	$registrasi = $db->database_num_rows($db->database_prepare("SELECT * FROM as_registrasi WHERE id_mhs = ? AND id_kelas = ? AND for_semester = ? AND status = 'Y'")->execute($data_mhs["id_mhs"],$data_mhs["kelas_id"],$sem));
	if ($registrasi > 0){
	...
	echo "<form method='POST' action='modul/mod_krs/aksi_krs.php?mod=krs&act=input'>
		<h5>Registrasi Ulang Semester</h5>
		<div class='box round first fullpage'>
		<div class='block '>
			<form method='POST' action='modul/mod_registrasi/aksi_registrasi.php'>
			<table class='form'>
				<tr>
					<td colspan='2'><b>Info Status Kuliah Saat Ini:</b></td>
				</tr>
				<tr>
					<td width='100'>NIM</td>
				...
				<tr>
					<td>Semester</td>
					<td><b>$data_mhs[semester_now]</b></td>
				</tr>
			</table>
			
			<table class='form'>
				<tr>
					<td colspan='2'><b>Registrasi Ulang</b></td>
				</tr>
				<tr>
					<td width='100'>Tanggal Registrasi</td>
					<td><b>$date_now</b></td>
				</tr>
				<tr>
					<td>Registrasi Sms</td>
					<td><b>$sem</b></td>
				</tr>
				<tr>
					<td>Status</td>
			...
			if ($registrasi == 0){
				echo "<button type='submit' class='btn btn-primary'>Registrasi</button>";
			}
			echo "</form>
		</div></div>";
	break;
